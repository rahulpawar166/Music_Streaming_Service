...
const PlayList = () => {
  ...
  const [currentPlaylist, setCurrentPlaylist] = useState(
    window.localStorage.getItem("currentPlaylist"),
  );
  ...
  const addPlaylist = async (event) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `http://localhost:3008/playlist/addPlaylist`,
        { uid: window.localStorage.getItem("currentUser"), name: formData },
      );
      console.log(data);
  ...
  const deletePlaylist = async (name_of_playlist) => {
    try {
      ...
      // console.log(formData,window.localStorage.getItem("currentUser"))
      const { data } = await axios.post(
        `http://localhost:3008/playlist/deletePlaylist`,
        {
          uid: window.localStorage.getItem("currentUser"),
        ...
        <Card className={classes.card} variant="outlined">
          ...
          {currentPlaylist === ind?.name ? (
            ...
            <Button
              ...
              onClick={() => changeCurrent(ind?.name)}
            >
              {" "}
              Make Active
            </Button>
          ...
          {ind?.name === "mydefault" ? null : (
            <Button onClick={() => deletePlaylist(ind?.name)}>
              {" "}
              <DeleteIcon />
            </Button>
          )}
