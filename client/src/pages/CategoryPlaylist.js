      ...
      <div key={trackAlbums.id}>
        ...
        <TableContainer container="true" className={classes.tableContainer} spacing={5}>
          {/* {albumDetails.tracks.items.map((track) => */}
            ...
            <h2 className={classes.subTitle} style={{ textAlign: 'center' }}>Tracks</h2>
            <List className={classes.list} style={{ marginTop: '30px' }}>
                {trackAlbums.map((element, idx) => (
                  <ListItem className={classes.listItem} key={element?.track.name}>
                      <ListItemText className={classes.link} style={{ maxWidth: '25px' }}>{idx + 1}.</ListItemText>
                      <ListItemText className={classes.link} style={{maxWidth: '1150px', textAlign: 'start' }} >{element?.track.name} - {element?.track.artists[0].name}</ListItemText>
                      <Button className={classes.addToPlaylistBtn} style={{textAlign: 'start'}} onClick={() => addToPlaylist(element?.track.id,element?.track.name)}>
                        Add To PlayList
                      </Button><br/>
                      <Button className={classes.playBtn} onClick={() => handlePlayingTrack(element?.track)}>
                        Play
                      </Button><br/>
                      <Button className={classes.lyricsBtn} href={`/Lyrics/${trackAlbums?.artists?.name}/${element?.track.name}`}>Lyrics</Button>         
                  </ListItem>
                ))}
                </List>
            </div>
